...
export function DualBarChart({
  ...
  data: Array<{ label: string; primary: number; secondary: number; sublabel?: string }>;
  primaryLabel: string;
        ...
        <div key={`${item.label}-${index}`} className="grid gap-2 rounded-lg border border-slate-100 bg-white px-3 py-2.5">
          <div className="flex items-start justify-between gap-3 text-sm">
            <span className="min-w-0">
              <span className="block truncate font-semibold text-slate-800" title={item.label}>{item.label}</span>
              {item.sublabel ? <span className="mt-0.5 block truncate text-[11px] font-semibold text-sky-700" title={item.sublabel}>{item.sublabel}</span> : null}
            </span>
